...
description: Allgemeine Hinweise zur Verwendung des Adressbuch-Addons von OpenEstate-ImmoTool…
weight: 20
...
1.  Klicken Sie im Hauptmenü auf den Eintrag `Adressbuch` → `Adressen` um die Adressübersicht zu öffnen.
...
Oberhalb der Tabelle werden verschiedene Eingabefelder dargestellt, um den Adressbestand eingrenzen zu können. Die Ansicht der Tabelle wird automatisch aktualisiert sobald ein Kriterium geändert wurde.
...
Klicken Sie im Hauptmenü auf `Adressen` → `Neu` → `Adresse`. Alternativ können Sie die [Adressübersicht](#usage_addressbook_manage_table) öffnen und oben rechts auf den Button `Neu` klicken.
...
Öffnen Sie die [Adressübersicht](#usage_addressbook_manage_table) und suchen Sie die zu bearbeitende Adresse in der Tabelle. Durch einen Doppelklick auf die betreffende Adresse wird das [Adressformular](#usage_addressbook_manage_form) zur Bearbeitung der Adresse geöffnet.
...
Öffnen Sie die [Adressübersicht](#usage_addressbook_manage_table) und suchen Sie die zu duplizierende Adresse in der Tabelle. Klicken Sie mit der rechten Maustaste auf die betreffende Adresse und wählen Sie im dargestellten Menü die Aktion `Adresse duplizieren`.

> **Hinweis**
>
> Die Originaladresse bleibt bei dieser Verfahrensweise unangetastet!


### Adresse löschen {#usage_addressbook_manage_remove}

Öffnen Sie die [Adressübersicht](#usage_addressbook_manage_table) und suchen Sie die zu löschende Adresse in der Tabelle. Klicken Sie mit der rechten Maustaste auf die betreffende Adresse und wählen Sie im dargestellten Menü die Aktion `Adresse löschen`.
...
Im Karteireiter `Allgemein` werden allgemeinen Eckdaten einer Adresse zusammengefasst.
...
Im Karteireiter `Details` können weitere Angaben wie z.B. Steuernummern oder eine Bankverbindung eingesehen / erfasst werden.


#### Verknüpfte Immobilien {#usage_addressbook_manage_form_realestate}

Im Karteireiter `Immobilien` können die mit der Adresse verknüpften Immobilien eingesehen / erfasst werden. Allgemein kann jeder Adressdatensatz kann mit einer oder mehreren Immobilien verknüpft werden.
...
Um eine Immobilienverknüpfung zu löschen, markieren Sie die entsprechende Immobilie mit der Maus. Klicken Sie mit der rechten Maustaste oder klicken Sie auf den Button `Aktionen` und wählen Sie die Aktion `Zuordnung löschen` aus.


#### Verknüpfte Kalendereinträge {#usage_addressbook_manage_form_calendar}

Im Karteireiter `Kalender` können die mit der Adresse verknüpften Kalendereinträge (Aufgaben oder Termine) eingesehen / erfasst werden. Allgemein kann jeder Adressdatensatz kann mit einem oder mehreren Kalendereinträgen verknüpft werden.

Klicken Sie auf `Neu` um eine neue Verknüpfung zu erfassen oder öffnen Sie eine bestehende Verknüpfung per Doppelklick in die Tabelle. Es öffnet sich daraufhin das folgende Fenster:

{{< figure src="address_assign_calendar.jpg" caption="Verknüpfung einer Immobilie mit einem Kalendereintrag" >}}

Wählen Sie die `Gruppe` und den `Eintrag` des zu verknüpfenden Kalendeereintrags aus. Klicken Sie abschließend auf `Übernehmen` um die bearbeite Verknüpfung ins Adressformular zu übernehmen.

Um eine Kalenderverknüpfung zu löschen, markieren Sie den entsprechenden Kalendereintrag mit der Maus. Klicken Sie mit der rechten Maustaste oder klicken Sie auf den Button `Aktionen` und wählen Sie die Aktion `Zuordnung löschen` aus.


#### Notizen {#usage_addressbook_manage_form_notes}

Im Karteireiter `Notizen` kann zu der Adresse ein beliebiger Notiztext hinterlegt werden.
...
Mit der Grundinstallation des Programms werden die Adressgruppen **privat** und **geschäftlich** in das Programm integriert. Bei Bedarf können diese Gruppen entfernt oder umbenannt werden - bzw. es können weitere Adressgruppen erfasst werden.
...
Nehmen Sie die gewünschten Einträge im [Adressgruppenformular](#usage_addressbook_manage_groups_form) vor und klicken Sie abschließend auf den Button `Speichern`.


### Adressgruppe bearbeiten {#usage_addressbook_manage_groups_edit}

Öffnen Sie die Adressansicht in der Sidebar, klicken Sie mit der rechten Maustaste auf die zu bearbeitende Adressgruppe und wählen Sie im Menü die Aktion `Adressgruppe bearbeiten`.
...
Öffnen Sie die Adressansicht in der Sidebar, klicken Sie mit der rechten Maustaste auf die zu löschende Adressgruppe und wählen Sie im Menü die Aktion `Adressgruppe löschen`.
...
### Adressgruppenformular {#usage_addressbook_manage_groups_form}

Im Adressgruppenformular können die Details zu einer Adressgruppe eingesehen und ggf. bearbeitet werden.
